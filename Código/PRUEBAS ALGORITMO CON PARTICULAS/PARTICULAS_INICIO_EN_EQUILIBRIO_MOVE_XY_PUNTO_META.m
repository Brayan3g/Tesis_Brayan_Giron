...
espacio = 4;             % Tamaño del espacio a utilizar en ambas dimensiones (de -espacio a espacio)
...
tiempo_movimiento = 2;  % Tiempo después del cual la partícula 1 comenzará a moverse
velocidad_lenta = 0.5;  % Velocidad lenta de la partícula 1 en metros/segundo
posicion_final = [3.75, 0];  % Posición final en metros en el espacio XY

% Configuración inicial de las partículas
num_particulas = 5;  % Número de partículas en la simulación (cambiar este valor para más partículas)
posicion_inicial = [-1, -1];  % Posición inicial de la primera partícula

% Opción de alineación: 'horizontal' o 'vertical'
alineacion = 'horizontal';  % Cambiar a 'vertical' para alinear las partículas en el eje Y
...
for paso = 1:num_pasos
    ...
    if usar_equilibrio && tiempo_actual > tiempo_movimiento
        % Calcular el vector dirección hacia la posición final
        direccion = posicion_final - posiciones(1, :);
        distancia = norm(direccion);
        
        % Normalizar el vector dirección y multiplicar por la velocidad lenta
        if distancia > 0
            direccion_normalizada = direccion / distancia;
            velocidades(1, :) = velocidad_lenta * direccion_normalizada;
        else
            velocidades(1, :) = [0, 0];  % Detener la partícula 1 si ha alcanzado la posición final
        end
    end
