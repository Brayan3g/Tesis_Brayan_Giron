...
posicion_inicial = [0, -1.2];  % Posición inicial de la primera partícula
...
for i = 2:num_particulas
    posiciones(i, :) = posiciones(i-1, :) - [distancia_inicial, 0];  % Colocar cada partícula a la misma distancia
end
...
for paso = 1:num_pasos
    ...
    if usar_equilibrio && tiempo_actual > tiempo_movimiento
        if posiciones(1, 1) < posicion_final
            velocidades(1, 1) = velocidad_lenta;  % Asignar una velocidad lenta a la partícula 1 en el eje X
        else
